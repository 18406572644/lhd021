...
public class ClaimRecordController {
    ...
    public Result<PageResult<ClaimRecord>> page(@ModelAttribute PageQuery query,
        ...
        Integer statusInt = StatusConverter.getPickupStatus(pickupStatus);
        return Result.success(claimRecordService.page(query, statusInt, keyword));
    }
    ...
    public Result<PageResult<ClaimRecord>> myClaim(@ModelAttribute PageQuery query,
                                                   @RequestParam(required = false) String pickupStatus) {
        Integer statusInt = StatusConverter.getPickupStatus(pickupStatus);
        return Result.success(claimRecordService.myClaim(query, statusInt));
    }
